...
/** Row from the same ordered list sent to Gemini; used to validate candidateIndex. */
function getViewportClickableRowAtIndex(index) {
  const list = getOrderedViewportClickables(IG_MAX_CANDIDATES);
...
/**
 * Only trust candidateIndex when it agrees with elementLabel and/or vision x/y.
 * Otherwise the model can pick a wrong index and we would highlight the wrong control.
 */
function candidateIndexIsPlausible(row, elementLabel, vx, vy) {
  if (!row) return false;
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const dist = Math.hypot(vx - row.cx, vy - row.cy);
  const maxDist = Math.min(vw, vh) * 0.24;

  const cand = {
    text: row.label,
    aria: row.el.getAttribute('aria-label') || '',
    title: row.el.getAttribute('title') || '',
  };
  const score = labelMatchScore(normalizeLabel(elementLabel), cand);
...
function drawGhostMouse(xPct, yPct, description, elementLabel, intentText, candidateIndex) {
  ...
  if (Number.isFinite(ci) && ci >= 0) {
    const row = getViewportClickableRowAtIndex(ci);
    if (row && candidateIndexIsPlausible(row, elementLabel, vx, vy)) {
      target = row.el;
